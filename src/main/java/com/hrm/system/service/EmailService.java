...
public class EmailService {
    ...
    // Send Payroll Access OTP to Admin / SuperAdmin
    public void sendPayrollAccessOtpEmail(String toEmail, String userName, String code) {
        String safeName = HtmlUtils.htmlEscape(userName != null ? userName : "Administrator");
        String htmlContent = """
            <!DOCTYPE html>
            <html>
            <head><style>
                body { font-family: Arial, sans-serif; margin: 0; padding: 0; background: #f3f4f6; }
                .container { max-width: 560px; margin: 40px auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08); }
                .header { background: linear-gradient(135deg, #4F46E5 0%, #3730A3 100%); color: white; padding: 32px 24px; text-align: center; }
                .header h2 { margin: 0; font-size: 22px; font-weight: 700; letter-spacing: 0.5px; }
                .header p { margin: 6px 0 0; font-size: 13px; color: rgba(255,255,255,0.8); }
                .content { padding: 32px 24px; color: #374151; }
                .otp-box { background: linear-gradient(135deg, #4F46E5 0%, #3730A3 100%); border-radius: 12px; padding: 24px; text-align: center; margin: 20px 0; }
                .otp-code { font-size: 42px; font-weight: 900; color: #ffffff; letter-spacing: 10px; font-family: 'Courier New', monospace; }
                .otp-label { font-size: 12px; color: rgba(255,255,255,0.8); margin-top: 6px; text-transform: uppercase; letter-spacing: 1.5px; }
                .info { background: #eef2ff; border-left: 4px solid #4F46E5; border-radius: 6px; padding: 14px 16px; font-size: 13px; color: #3730A3; margin-top: 18px; }
                .footer { text-align: center; padding: 20px; font-size: 11px; color: #9ca3af; background: #f9fafb; border-top: 1px solid #f3f4f6; }
            </style></head>
            <body>
                <div class="container">
                    <div class="header">
                        <h2>🔒 Payroll Access Code</h2>
                    ...
                    <div class="content">
                        ...
                        <p style="margin-top:20px; color: #6b7280; font-size: 13px;">If you did not request access to Payroll Processing, please alert system administrators immediately.</p>
                        <p>Regards,<br><strong>JCAT Solutions HRM Security</strong></p>
                    </div>
                    <div class="footer">© 2026 JCAT Solutions HRM. All rights reserved.</div>
                </div>
            </body>
        ...
        String apiKey = System.getenv("RESEND_API_KEY");
        if (apiKey != null && !apiKey.trim().isEmpty()) {
            sendViaResend(toEmail, "Your Payroll Access Code - " + code, htmlContent, true);
            return;
        }
        ...
        try {
            ...
            helper.setFrom(fromEmail);
            helper.setTo(toEmail);
            helper.setSubject("Your Payroll Access Code - " + code);
            helper.setText(htmlContent, true);
            mailSender.send(message);
            log.info("✓ Payroll access OTP email sent via SMTP to: {}", toEmail);
        } catch (Exception e) {
            log.error("✗ Failed to send payroll access OTP email to: {}: {}", toEmail, e.getMessage(), e);
            throw new RuntimeException("Email sending failed: " + e.getMessage(), e);
        }
    }
