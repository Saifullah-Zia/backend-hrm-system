...
public class StartupMaintenanceService {
    ...
    public void runDeferredStartupMaintenance() {
        try {
            ...
            jdbcTemplate.execute("UPDATE leave_balances lb SET total_days = lp.total_days_per_year FROM leave_policies lp WHERE lb.leave_type = lp.leave_type AND lb.used_days = 0 AND lb.pending_days = 0");
            System.out.println("✅ Legacy database columns cleanup and leave policy sync finished successfully.");
        } catch (Exception e) {
